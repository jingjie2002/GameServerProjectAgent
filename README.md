...
- [轻量级服务器部署与管理约定](docs/server-deployment.md)
## Setup 向导
...
阶段 11 增加了轻量级服务器部署约定预览：

```powershell
.\bin\gsa.exe server plan
```

该命令只读展示当前本机目录、建议的 Linux 服务器目录和后续操作顺序，不创建目录，不启动服务。可以指定服务器根目录预览：

```powershell
.\bin\gsa.exe server plan --root /opt/gsa
```
...
```text
GSA_HOME=/opt/gsa
GSA_WORKSPACE=/srv/gsa/workspace
GSA_CONFIG=/opt/gsa/.gsa/config.yaml
```

当前阶段仍不直接执行 systemd / Docker Compose，不开放公网端口，不自动执行数据库迁移。

## 只读 Web 状态面板

阶段 11 增加了本地可预览的只读状态面板：

```powershell
.\bin\gsa.exe dashboard --host 127.0.0.1 --port 18088
```
...
- 已注册模块列表。
- deploy 状态、PID、端口和日志入口。
- health / capabilities / metrics 只读状态。
- `/api/status` JSON 状态接口。
- `/api/logs/<project_id>?tail=120` 日志尾部接口。

第一版 Web 面板只读展示，不提供启动、停止或重启按钮。真正启停服务仍必须使用 CLI，并显式传入：
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\dashboard_smoke.ps1
```

脚本会启动面板、检查 `/healthz`、`/api/status` 和首页，随后停止进程；成功后自动删除 `tmp\dashboard-smoke.*`，失败时保留文件便于排查。
